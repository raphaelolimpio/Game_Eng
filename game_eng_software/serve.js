...
app.get('/users', (req, res) => {
  const query = 'SELECT idUser, nome, email, registration, points, level, idFunction, idGroup, idClass FROM users';
...
app.get('/users/:idUser', (req, res) => {
  ...
  const query = 'SELECT idUser, nome, email, registration, points, level, idFunction, idGroup, idClass FROM users WHERE idUser = ?';
...
app.get('/master', (req, res) => {
  const query = 'SELECT idMaster, nome, email FROM master';
  db.query(query, (err, results) => {
    if (err) {
      console.error('Erro na query:', err);
...
app.get('/master/:idMaster', (req, res) => {
  ...
  const query = 'SELECT idMaster, nome, email, senha, idClass FROM master WHERE idMaster = ?';
...
app.get('/match', (res) => {
  const query = 'SELECT idMatch, idMaster, data_inicio, estado_partida FROM match';
  db.query(query, (err, results) => {
    if (err) {
      console.error('Erro na query:', err);
...
app.get('/match/:idMatch', (req, res) => {
  ...
  const query = 'SELECT idMatch, idMaster, data_inicio, estado_partida FROM match WHERE idMatch = ?';
...
app.get('/match_group', (res) => {
  const query = 'SELECT idMatch, idGroup, pontuacao_match, posicao_rank FROM match_group';
  db.query(query, (err, results) => {
    if (err) {
      console.error('Erro na query:', err);
      res.status(500).send('Erro no servidor ao buscar grupos.');
...
app.get('/quests', (req, res) => {
  const query = 'SELECT idQuest, idFunction, text_quest, level FROM quests';
  db.query(query, (err, results) => {
    if (err) {
      console.error('Erro na query:', err);
...
app.get('/quest/:idQuest', (req, res) => {
  const { idQuest } = req.params;
  const query = 'SELECT idQuest, idFunction, text_quest, level FROM quests WHERE idQuest = ?';
...
app.delete('/quest/:idQuest', (req, res) => {
  const { idQuest } = req.params;
...
app.get('/alternatives_quest', (req, res) => {
  const query = 'SELECT idAlternatives, idQuest, text_alternative, isTrue FROM alternatives_quest';
  db.query(query, (err, results) => {
    if (err) {
      console.error('Erro na query:', err);
...
app.get('/alternatives_quest/:idQuest', (req, res) => {
  const { idQuest } = req.params;
  const query = 'SELECT idAlternatives, idQuest, text_alternative, isTrue FROM alternatives_quest WHERE idQuest = ?';
  db.query(query, [idQuest], (err, results) => {
    if (err) {
...
app.get('/answers', (req, res) => {
  const query = 'SELECT idAnswers, idMatch, idQuest, idUser, idGroup, answers, isTrue, pontos_conquistados, timestamp FROM answers';
  db.query(query, (err, results) => {
    if (err) {
      console.error('Erro na query:', err);
...
app.get('/answers/:idQuest', (req, res) => {
  const { idQuest } = req.params;
  const query = 'SELECT idAnswers, idMatch, idQuest, idUser, idGroup, answers, isTrue, pontos_conquistados, timestamp FROM answers WHERE idQuest = ?';
  db.query(query, [idQuest], (err, results) => {
    if (err) {
...
app.post('/game_card', (req, res) => {
  ...
  db.query(query, [idCard, title_card, description], (err, result) => {
    if (err) {
...
app.get('/game_card', (req, res) => {
  const query = 'SELECT idCard, title_card, description FROM game_card';
  db.query(query, (err, results) => {
    if (err) {
      console.error('Erro na query:', err);
...
app.get('/game_card/:idCard', (req, res) => {
  ...
  const query = 'SELECT idCard, title_card, description FROM game_card WHERE idCard = ?';
...
app.put('/game_card/:idCard', (req, res) => {
  const { idCard } = req.params;
  const { title_card, description } = req.body;
  const query = 'UPDATE game_card SET title_card = ?, description = ? WHERE idCard = ?';
  db.query(query, [title_card, description, idCard], (err, result) => {
    if (err) {
